...
/*	specifically:
		fprintf(stderr,...
		printf in faidx_main (excluded by #ifdef FAIDX_MAIN)
*/
...
faidx_t *fai_build_core(RAZF *rz)
{
        ...
        while (razf_read(rz, &c, 1)) {
                ...
                if (c == '>') { // fasta header
                        ...
                        name[l_name] = '\0';
                        if (ret == 0) {
                                PRINT_ERROR("[fai_build_core] the last entry has no sequence\n");
                        ...
                        if (state == 3) {
                                PRINT_ERROR("[fai_build_core] inlined empty line is not allowed in sequence '%s'.\n", name);
                                free(name); fai_destroy(idx);
                                return 0;
                        }
                        if (state == 2) state = 3;
                        ...
                        if (state == 3 && l2) {
                                PRINT_ERROR("[fai_build_core] different line length in sequence '%s'.\n", name);
                                free(name); fai_destroy(idx);
                                return 0;
                        }
                        ++l1; len += l2;
...
int fai_build(const char *fn)
{
        ...
        rz = razf_open(fn, "r");
        if (rz == 0) {
                PRINT_ERROR("[fai_build] fail to open the FASTA file %s\n",fn);
                free(str);
                return -1;
        }
        ...
        fp = fopen(str, "wb");
        if (fp == 0) {
                PRINT_ERROR("[fai_build] fail to write FASTA index %s\n",str);
...
FILE *download_and_open(const char *fn)
{
    ...
    if ((fp = fopen(fn, "wb")) == 0) {
        PRINT_ERROR("[download_from_remote] fail to create file in the working directory %s\n",fn);
...
faidx_t *fai_load(const char *fn)
{
    ...
    if (strstr(fn, "ftp://") == fn || strstr(fn, "http://") == fn)
    {
        ...
        if ( !fp )
        {
            PRINT_ERROR("[fai_load] failed to open remote FASTA index %s\n", str);
            free(str);
            return 0;
        }
    }
        ...
        fp = fopen(str, "rb");
        if (fp == 0) {
                PRINT_ERROR("[fai_load] build FASTA index.\n");
                fai_build(fn);
                fp = fopen(str, "rb");
                if (fp == 0) {
                        PRINT_ERROR("[fai_load] fail to open FASTA index.\n");
                        free(str);
                        return 0;
                }
        }
        ...
        free(str);
        if (fai->rz == 0) {
                PRINT_ERROR("[fai_load] fail to open FASTA file.\n");
                return 0;
        }
        return fai;
}
...
char *fai_fetch(const faidx_t *fai, const char *str, int *len)
{
        ...
        if(iter == kh_end(h)) {
                PRINT_ERROR("[fai_fetch] Warning - Reference %s not found in FASTA file, returning empty sequence\n", str);
                free(s);
                return 0;
        };
        val = kh_value(h, iter);
...
{
        if (argc == 1) {
                PRINT_ERROR("Usage: faidx <in.fasta> [<reg> [...]]\n");
                return 1;
        } else {
