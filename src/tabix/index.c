...
#define		WHOP_ASSERT( expr )					if( !(expr) ) Rprintf("ASSERTION FAILED: " #expr " !\n" );
#define		PRINT_ERROR							Rprintf
...
static int get_tid(ti_index_t *idx, const char *ss)
{
	...
	if (k == kh_end(idx->tname)) { /* a new target sequence*/
		...
		WHOP_ASSERT(idx->n == kh_size(idx->tname));
...
/*
	if (ncols < conf->sc || ncols < conf->bc || ncols < conf->ec) {
		if (ncols == 1) (stderr,"[get_intv] Is the file tab-delimited? The line has %d field only: %s\n", ncols, line);
...
static int get_intv(ti_index_t *idx, kstring_t *str, ti_intv_t *intv)
{
	...
	if (ti_get_intv(&idx->conf, str->l, str->s, &x) == 0) {
		...
		*x.se = '\0'; intv->tid = get_tid(idx, x.ss); *x.se = c;
		...
		PRINT_ERROR( "[%s] the following line cannot be parsed and skipped: %s\n", __func__, str->s);
		return -1;
	}
}
...
static ti_index_t *ti_index_load_core(BGZF *fp)
{
	...
	ti_is_be = bam_is_big_endian();
	if (fp == 0) {
		PRINT_ERROR( "[ti_index_load_core] fail to load index.\n");
		return 0;
	}
	bgzf_read(fp, magic, 4);
	if (strncmp(magic, "TBI\1", 4)) {
		PRINT_ERROR( "[ti_index_load] wrong magic number.\n");
		return 0;
	}
	idx = (ti_index_t*)calloc(1, sizeof(ti_index_t));	
...
static void download_from_remote(const char *url)
{
	...
	if ((fp = fopen(fn, "w")) == 0) {
		PRINT_ERROR( "[download_from_remote] fail to create file in the working directory.\n");
...
static char *get_local_version(const char *fn)
{
	...
	if ((strstr(fnidx, "ftp://") == fnidx || strstr(fnidx, "http://") == fnidx)) {
		...
		PRINT_ERROR( "[%s] downloading the index file...\n", __func__);
...
/*!	Allocates and returns an array of pointers to Contig/Chromosome names used in the tabixed file
**
*/
...
ti_index_t *ti_index_load(const char *fn)
{
	...
	if (idx == 0) PRINT_ERROR( "[ti_index_load] fail to load the index: %s\n", fname);
    free(fname);
	return idx;
}
...
int ti_index_build2(const char *fn, const ti_conf_t *conf, const char *_fnidx)
{
	...
	if ((fp = bgzf_open(fn, "r")) == 0) {
		PRINT_ERROR( "[ti_index_build2] fail to open the file: %s\n", fn);
		return -1;
	}
	...
	if (fpidx == 0) {
		PRINT_ERROR( "[ti_index_build2] fail to create the index file.\n");
		free(fnidx);
		return -1;
	}
...
ti_iter_t ti_iter_query(const ti_index_t *idx, int tid, int beg, int end)
{
	...
	{
		min_off = (beg>>TAD_LIDX_SHIFT >= idx->index2[tid].n) ?
						idx->index2[tid].offset[idx->index2[tid].n-1]
						:
...
tabix_t *ti_open(const char *fn, const char *fnidx)
{
	tabix_t *t;
	BGZF *fp;
	ONDBG Rprintf("bgzf_open(%s,\"r\")...\n",fn);
	if ((fp = bgzf_open(fn, "r")) == 0){ ONDBG Rprintf("...bgzfopen=FAIL\n"); return 0; }
	ONDBG Rprintf("...bgzfopen=OK\n");
	ONDBG Rprintf("calloc(1,%d)\n",sizeof(tabix_t));
...
ti_iter_t ti_query(tabix_t *t, const char *name, int beg, int end)
{
	...
	if ((tid = ti_get_tid(t->idx, name)) < 0)
		return 0;
