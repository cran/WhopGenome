...
{	
	...
	vcff * f = (vcff*)R_GetExtPtr( vcfptr , "VCFhandle" );
	if( 0 == f ) {
		df0("VCF_getSampleByName : parameter 1 is not a valid VCFhandle!\n");
		return R_NilValue;
	}
	
	//	make sure sample index is an integer
	//
	if( RString::isStr( str_samplename ) == false ) {
		df0("VCF_getSampleByName : parameter 2 is not a string!\n");
		return R_NilValue;
	}

	//	get as int and make sure its inside the valid range
	//
	const char * smplname = RString::get(str_samplename);
	unsigned int smplidx = f->getSampleIndexByName( smplname );
	
	//
	if( (f->getNumFields() < smplidx) || smplidx == 0 ) {
		df1("(WARNING) index %d for sample of name '%s'!\n",smplidx,smplname);
		return R_NilValue;
	}
	...
	RString resvec;
	char	buffer[512];
	if( f->copyField( smplidx, &buffer[0], sizeof(buffer)-2 ) )
	{
		resvec.alloc( 1 );
		resvec.set( &buffer[0] );
		return resvec.get();
